...
var sa_admin = new Vue({
  ...
  methods: {
    ...
    init: function(option) {
      ...
      // 初始化 swiper
      var switchV =
        localStorage.getItem('switchV') || option.switchDefault || 'fade';
      ...
      this.is_show_tabbar =
        option.is_show_tabbar === undefined
          ? this.is_show_tabbar
          : option.is_show_tabbar; // 是否显示tabbar栏
      this.is_reme_open =
        option.is_reme_open === undefined
          ? this.is_reme_open
          : option.is_reme_open; // 是否记住上一次最后打开的窗口
    ...
    right_xf: function() {
      ...
      document.querySelector(
        '#layui-layer' + index + ' .layui-layer-max'
      ).onclick = function() {
        setTimeout(function() {
          sa_admin_code_util.solveLayerBug(index);
        }, 200);
    ...
    getSlideIndexById: function(id) {
      ...
      if (iframe != null) {
        ...
        var slideIndex = [].indexOf.call(
          slide.parentNode.querySelectorAll('.swiper-slide'),
          slide
        );
        return slideIndex;
      }
    ...
    printVesion: function() {
      ...
      console.log(
        '如在使用中发现任何bug或者疑问，请加入QQ群交流：782974737，点击加入：' +
